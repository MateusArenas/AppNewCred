...
const ChatBot = props => {
  ...
  const checkInvalidInput = () => {
    ...
    React.useEffect(() => {
      if (editable) {
        const first = setTimeout(() => {
          ...
          pressableRef?.current?.focus?.();
          pressableRef?.current?.getElement?.()?.focus?.();
    ...
    React.useEffect(() => {
      if ((!!currentStep?.type && currentStep?.type !== "default") && !!currentStep?.options) {
        if (Platform.OS === "web") {
          pressableRef?.current?.focus?.()
        }
      }
    ...
    return (
      <>
        <OptionsModal 
          ...
          state={state}
          visible={(!!currentStep?.type && currentStep?.type !== "default") && modalVisible}
          onDismiss={() => setModalVisible(false)}
          onSubmit={data => triggerNextStep({ ...data })}
        />
        <ChatBotContainer
        ...
        >
          ...
          >
            <Footer
              ...
              color={'rgba(0,0,0,.1)'}
            >
              {(!!inputValue && !!currentStep?.suggestions) && (
                <InputSuggestions 
                  suggestions={currentStep?.suggestions}
                  value={inputValue}
                  onChangeSuggestion={suggestion => setState(state => ({ ...state, inputValue: suggestion }))}
                />
              )}
              {((!!currentStep?.type && currentStep?.type !== "default") && !!currentStep?.options) ? (
                ...
                <>
                  ...
                  >
                    {props?.submitButtonTextComponent ? 
                      // props?.submitButtonTextComponent({
                      //   className: "rsc-button-custom-text",
                      //   invalid: inputInvalid,
                      //   fontColor: botFontColor
                      // })
                      <SendIcon />
...
ChatBot.defaultProps = {
  ...
  userBubbleStyle: {},
  userBubbleColor: '#ebeff5',
  userDelay: 1000,
  userFontColor: '#4a4a4a',
  // botAvatar: Image.resolveAssetSource(require('../../assets/images/avatar.png')).uri,
  botAvatar: "",
  ...
  botAvatar: "",
  userAvatar: "",
  scrollViewProps: { position: 'relative' },
};
...
const ChatbotTextInput = React.forwardRef((props, ref) => {
  ...
  return <TextInputMask ref={ref} {...props}/>;
})


const RenderStep = React.memo(({ step, state, triggerNextStep, isFirstPosition, isLastPosition, index, ...props }) => {
  ...
  const { options, component, event, asMessage } = step;
  const steps = {};
  ...
  if (options || step?.freezes) {
    if (step?.type && step?.type !== "default") return null;
    return (
      <OptionsStep
      index={index}
